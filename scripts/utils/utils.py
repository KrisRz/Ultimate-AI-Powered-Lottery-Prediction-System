...
def setup_logging():
    ...
    lottery_log = log_dir / "lottery.log"
    
    # Create file handlers
    training_handler = logging.FileHandler(log_file)
    lottery_handler = logging.FileHandler(lottery_log)
    console_handler = logging.StreamHandler(sys.stdout)
    
    # Set formatters
    formatter = logging.Formatter('%(asctime)s - %(levelname)s - [%(name)s] - %(message)s')
    training_handler.setFormatter(formatter)
    lottery_handler.setFormatter(formatter)
    ...
    root_logger.addHandler(lottery_handler)
    root_logger.addHandler(console_handler)
    
    # Configure specific loggers
    for logger_name in ['models', 'scripts', 'utils']:
        logger = logging.getLogger(logger_name)
        logger.setLevel(logging.INFO)
        logger.addHandler(training_handler)
        logger.addHandler(lottery_handler)
