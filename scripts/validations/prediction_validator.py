...
# Define LOG_DIR directly instead of importing from scripts.utils to avoid circular imports
ROOT_DIR = Path(__file__).parent.parent.parent
LOG_DIR = ROOT_DIR / "logs"
LOG_DIR.mkdir(exist_ok=True, parents=True)

# Setup logging directly
def setup_validation_logging(log_level=logging.INFO):
    """Set up logging configuration specifically for validation."""
    logging.basicConfig(
        level=log_level,
...
class LotteryValidator:
    ...
    def validate_predictions(self, predictions: List[List[int]], n_monte_carlo: int = 1000) -> Dict[str, Any]:
        ...
        try:
            ...
            # Backtesting
            results['backtest'] = self._calculate_backtest_metrics(predictions)
            
            # Monte Carlo simulation
            results['monte_carlo'] = self._monte_carlo_simulation(n_monte_carlo)
            
            # Calculate improvement over random
            results['improvement'] = {
                'match_rate': results['backtest']['match_rate'] - results['monte_carlo']['random_match_rate'],
                'exact_rate': results['backtest']['exact_match_rate'] - results['monte_carlo']['random_exact_rate']
            }
            
            # Distribution analysis
            results['distribution'] = self._analyze_distribution(predictions)
            
            # Consistency analysis
            results['consistency'] = self._analyze_consistency(predictions)
