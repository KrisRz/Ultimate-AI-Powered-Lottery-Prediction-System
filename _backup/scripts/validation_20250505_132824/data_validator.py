...
class DataValidator:
    ...
    def __init__(self, validation_rules: Optional[Dict] = None):
        ...
        self.history_file = Path('outputs/outputs/results/validation_log.json')
    ...
    def validate_data(self, data: pd.DataFrame, fix_issues: bool = True) -> Tuple[bool, Dict]:
        ...
        Returns:
            Tuple of (is_valid: bool, outputs/results: Dict) indicating validation success and details
        """
        start_time = datetime.now()
        validation_outputs/results = {
        ...
        try:
            ...
            if len(df) == 0:
                validation_outputs/results['errors'].append("Empty dataframe - no data to validate")
                self._save_validation_outputs/results(validation_outputs/results)
                return False, validation_outputs/results
            
            # Check required columns
            required_cols = {k for k, v in self.validation_rules.items() if v.get('required', False)}
            missing_cols = required_cols - set(df.columns)
            if missing_cols:
                validation_outputs/results['errors'].append(f"Missing required columns: {missing_cols}")
            
            # Check for extra columns not in rules
            extra_cols = set(df.columns) - set(self.validation_rules.keys())
            if extra_cols:
                validation_outputs/results['warnings'].append(f"Extra columns found that are not in validation rules: {extra_cols}")
            ...
            if 'Balls' in df.columns and ('Main_Numbers' not in df.columns or 'Bonus' not in df.columns):
                if fix_issues:
                    ...
                    validation_outputs/results['fixes_applied'].append("Extracted Main_Numbers and Bonus from Balls column")
            ...
            if 'Winners' in df.columns:
                ...
                if invalid_winners.any():
                    ...
                    validation_outputs/results['errors'].append(f"Winners: Found {invalid_winners.sum()} values below minimum 0")
                    
                    if fix_issues:
                        df.loc[invalid_winners, 'Winners'] = 0
                        validation_outputs/results['fixes_applied'].append(f"Winners: Set {invalid_winners.sum()} invalid values to 0")
            ...
            if fix_issues:
                ...
                if 'Draw Details' in df.columns:
                    ...
                    if missing_draw_details.any():
                        ...
                        validation_outputs/results['fixes_applied'].append(f"Draw Details: Filled {missing_draw_details.sum()} missing values")
            ...
            for column, rules in self.validation_rules.items():
                if column not in df.columns:
                    if rules.get('required', False):
                        ...
                        validation_outputs/results['warnings'].append(f"Optional column {column} missing")
                        continue
                
                column_outputs/results = self._validate_column(df, column, rules, fix_issues)
                validation_outputs/results['checks'][column] = column_outputs/results
                
                if column_outputs/results.get('errors', []):
                    validation_outputs/results['errors'].extend([f"{column}: {e}" for e in column_outputs/results['errors']])
                if column_outputs/results.get('warnings', []):
                    validation_outputs/results['warnings'].extend([f"{column}: {w}" for w in column_outputs/results['warnings']])
                if column_outputs/results.get('fixes', []):
                    validation_outputs/results['fixes_applied'].extend([f"{column}: {f}" for f in column_outputs/results['fixes']])
            
            # Additional cross-column and consistency checks
            consistency_outputs/results = self._check_consistency(df)
            validation_outputs/results['checks']['consistency'] = consistency_outputs/results
            if consistency_outputs/results.get('errors', []):
                validation_outputs/results['errors'].extend([f"Consistency: {e}" for e in consistency_outputs/results['errors']])
            if consistency_outputs/results.get('warnings', []):
                validation_outputs/results['warnings'].extend([f"Consistency: {w}" for w in consistency_outputs/results['warnings']])
            ...
            # Save outputs/results and return
            self._save_validation_outputs/results(validation_outputs/results)
            
            # Log summary and update with duration
            duration = (datetime.now() - start_time).total_seconds()
            validation_outputs/results['duration_seconds'] = duration
            
            is_valid = len(validation_outputs/results['errors']) == 0
            logger.info(f"Data validation completed in {duration:.2f}s: {len(validation_outputs/results['errors'])} errors, "
                        f"{len(validation_outputs/results['warnings'])} warnings, "
                        f"{len(validation_outputs/results['fixes_applied'])} fixes")
            
            if validation_outputs/results['errors']:
                logger.error(f"Validation errors: {validation_outputs/results['errors']}")
            if validation_outputs/results['warnings']:
                logger.warning(f"Validation warnings: {validation_outputs/results['warnings']}")
            if validation_outputs/results['fixes_applied']:
                logger.info(f"Fixes applied: {validation_outputs/results['fixes_applied']}")
            
            return is_valid, validation_outputs/results
        
        except Exception as e:
            logger.error(f"Error during validation: {str(e)}")
            logger.error(traceback.format_exc())
            validation_outputs/results['errors'].append(f"Validation failed: {str(e)}")
            return False, validation_outputs/results
    ...
    def _validate_column(self, df: pd.DataFrame, column: str, rules: Dict, fix_issues: bool) -> Dict:
        ...
        Returns:
            Dictionary with validation outputs/results
        """
        series = df[column]
        outputs/results = {
            'errors': [],
        ...
        try:
            ...
            if 'type' not in rules:
                if column == 'Jackpot' and pd.api.types.is_string_dtype(series):
                    ...
                    if invalid_format.any():
                        ...
                        outputs/results['errors'].append(f"Found {invalid_format.sum()} values with invalid format. Examples: {invalid_examples}")
                        
                        if fix_issues:
                            for idx in df.index[invalid_format]:
                                df.at[idx, column] = '£0'
                            outputs/results['fixes'].append(f"Fixed {invalid_format.sum()} invalid jackpot values")
            ...
            if rules['type'] == 'datetime':
                        ...
                        try:
                            ...
                            if pd.api.types.is_string_dtype(series):
                                ...
                                if not invalid_dates.empty:
                                    outputs/results['errors'].append(f"Invalid date format in {len(invalid_dates)} values")
                                    
                            df[column] = pd.to_datetime(series, infer_datetime_format=True, errors='coerce')
                            outputs/results['fixes'].append(f"Converted to datetime (format auto-detected)")
                            
                            # Check for NaT values after conversion (indicates invalid dates)
                            if df[column].isna().any():
                                outputs/results['errors'].append(f"Failed to convert {df[column].isna().sum()} values to datetime")
                                
                        except Exception as e:
                            outputs/results['errors'].append(f"Failed to convert to datetime: {str(e)}")
                    else:
                        outputs/results['errors'].append("Column must be datetime type")
                ...
                if not pd.api.types.is_integer_dtype(series):
                    if fix_issues:
                        try:
                            ...
                            outputs/results['fixes'].append(f"Converted to integer (missing/invalid values set to 0)")
                        except Exception as e:
                            outputs/results['errors'].append(f"Failed to convert to integer: {str(e)}")
                    else:
                        outputs/results['errors'].append("Column must be integer type")
                ...
                if non_list_indices:
                    outputs/results['errors'].append(f"Found {len(non_list_indices)} values that are not lists at indices: "
            ...
            if rules.get('unique', False):
                ...
                if duplicates.any():
                    ...
                    outputs/results['warnings'].append(f"Found {duplicates.sum()} duplicate values: {dup_values[:3]}"
            ...
            if 'values' in rules:
                ...
                if invalid_mask.any():
                    ...
                    outputs/results['errors'].append(f"Found {invalid_mask.sum()} invalid values: {invalid_values[:3]}"
            ...
            if 'format' in rules and pd.api.types.is_string_dtype(series):
                ...
                if invalid_mask.any():
                    ...
                    outputs/results['errors'].append(f"Found {invalid_mask.sum()} values with invalid format. Examples: "
                                           f"{invalid_examples}")
                    ...
                    if fix_issues and column == 'Jackpot':
                        try:
                            for idx in df.index[invalid_mask]:
                                ...
                                df.at[idx, column] = '£0'
                            outputs/results['fixes'].append(f"Fixed {invalid_mask.sum()} invalid jackpot values")
                        except Exception as e:
                            outputs/results['errors'].append(f"Error fixing jackpot format: {str(e)}")
                ...
                if 'min' in rules:
                    ...
                    if below_min.any():
                        if fix_issues and rules.get('fix_min', True):  # Default to fixing min values
                            ...
                            outputs/results['fixes'].append(f"Set {below_min.sum()} values below minimum to {rules['min']}")
                        else:
                            outputs/results['errors'].append(f"Found {below_min.sum()} values below minimum {rules['min']}")
                ...
                if 'max' in rules:
                    ...
                    if above_max.any():
                        if fix_issues and rules.get('fix_max', True):  # Default to fixing max values
                            ...
                            outputs/results['fixes'].append(f"Set {above_max.sum()} values above maximum to {rules['max']}")
                        else:
                            outputs/results['errors'].append(f"Found {above_max.sum()} values above maximum {rules['max']}")
            ...
            if fix_issues and rules.get('fix_missing', False) and series.isna().any():
                ...
                if rules['type'] == 'int':
                    ...
                    outputs/results['fixes'].append(f"Filled {missing_count} missing values with 0")
                elif rules['type'] == 'str':
                    df[column] = series.fillna('')
                    outputs/results['fixes'].append(f"Filled {missing_count} missing values with empty string")
            
            # Custom validation function
            if 'validate' in rules:
                custom_errors, custom_warnings, custom_fixes = rules['validate'](df, column, fix_issues)
                outputs/results['errors'].extend(custom_errors)
                outputs/results['warnings'].extend(custom_warnings)
                outputs/results['fixes'].extend(custom_fixes)
            ...
            logger.error(traceback.format_exc())
            outputs/results['errors'].append(f"Validation error: {str(e)}")
            return outputs/results
    ...
    def _check_consistency(self, df: pd.DataFrame) -> Dict:
        ...
        Returns:
            Dictionary with consistency check outputs/results
        """
        outputs/results = {
            'errors': [],
            'warnings': []
        ...
        try:
            ...
            if 'Draw Date' in df.columns and pd.api.types.is_datetime64_any_dtype(df['Draw Date']):
                if not df['Draw Date'].is_monotonic_increasing:
                    ...
                    outputs/results['warnings'].append(f"Draw Date is not chronological at indices: {unsorted_indices[:5]}"
            ...
            if 'Draw Date' in df.columns and 'Day' in df.columns:
                ...
                if pd.api.types.is_datetime64_any_dtype(df['Draw Date']):
                    ...
                    if day_mismatches:
                        outputs/results['warnings'].append(f"Day of week doesn't match Draw Date for {len(day_mismatches)} rows: "
            ...
            for column, rules in self.validation_rules.items():
                if column in df.columns and rules.get('required', False):
                    ...
                    if missing > 0:
                        outputs/results['errors'].append(f"Required column {column} has {missing} missing values")
            ...
            if 'Balls' not in df.columns and not ('Main_Numbers' in df.columns and 'Bonus' in df.columns):
                outputs/results['errors'].append("Either 'Balls' or both 'Main_Numbers' and 'Bonus' must be present")
            ...
            for column in df.select_dtypes(include=[np.number]).columns:
                if column in self.validation_rules and len(df) > 10:  # Only check if we have enough data
                    ...
                    if len(outliers) > 0:
                        outputs/results['warnings'].append(f"Column {column} has {len(outliers)} extreme outliers")
            
            return outputs/results
            
        except Exception as e:
            logger.error(f"Error checking consistency: {str(e)}")
            outputs/results['errors'].append(f"Consistency check error: {str(e)}")
            return outputs/results
    ...
    def _save_validation_outputs/results(self, outputs/results: Dict):
        """
        Save validation outputs/results to history file.
        
        Args:
            outputs/results: Validation outputs/results to save
        """
        try:
            self.history_file.parent.mkdir(exist_ok=True)
            
            history = self.get_validation_history()
            history.append(outputs/results)
            
            # Keep only the last 10 validation outputs/results to prevent file growth
            if len(history) > 10:
                history = history[-10:]
            ...
            logger.error(f"Error saving validation outputs/results: {str(e)}")
    ...
    def validate_dataframe(self, df: pd.DataFrame, fix_issues: bool = True) -> Tuple[bool, Dict]:
        ...
        Returns:
            Tuple of (is_valid, validation_outputs/results)
        """
        try:
    ...
    def validate_jackpot_column(self, df: pd.DataFrame, fix_issues: bool = True) -> Tuple[bool, Dict]:
        ...
        Returns:
            Tuple of (is_valid: bool, outputs/results: Dict)
        """
        outputs/results = {
            'errors': [],
            'warnings': [],
        ...
        try:
            if 'Jackpot' not in df.columns:
                outputs/results['errors'].append("Jackpot column not found")
            ...
            if invalid_format.any():
                outputs/results['errors'].append(f"Found {invalid_format.sum()} values with invalid format")
                
                if fix_issues:
                    for idx in df.index[invalid_format]:
                        df.at[idx, 'Jackpot'] = '£0'
                    outputs/results['fixes_applied'].append(f"Fixed {invalid_format.sum()} invalid jackpot values")
            
            is_valid = len(outputs/results['errors']) == 0
            return is_valid, outputs/results
            
        except Exception as e:
            outputs/results['errors'].append(f"Error validating jackpot: {str(e)}")
            return False, outputs/results
...
if __name__ == "__main__":
    ...
    try:
        ...
        is_valid, outputs/results = validate_dataframe(df, fix_issues=args.fix)
        
        print(f"\nValidation {'PASSED' if is_valid else 'FAILED'}")
        print(f"Found {len(outputs/results['errors'])} errors, {len(outputs/results['warnings'])} warnings")
        
        if outputs/results['errors']:
            print("\nERRORS:")
            for e in outputs/results['errors']:
                print(f"  - {e}")
        
        if outputs/results['warnings']:
            print("\nWARNINGS:")
            for w in outputs/results['warnings']:
                print(f"  - {w}")
        
        if args.fix and outputs/results['fixes_applied']:
            print("\nFIXES APPLIED:")
            for f in outputs/results['fixes_applied']:
                print(f"  - {f}")
        
        print(f"\nValidation log saved to {Path('outputs/outputs/results/validation_log.json')}")
