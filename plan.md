...
- [ ] **Ujednolicić linie z maila i z `latest.json`.** `ev_play.py` buduje portfel bez ziarna, `ev_alert.py` seeduje datą losowania — przy PLAY obie ścieżki dają **różne** kupony, a `roi_ledger add --from-latest` zapisze te z `latest.json`, nie te z maila. Nie zapaliło się 08.08 tylko dlatego, że werdykt to SKIP.
